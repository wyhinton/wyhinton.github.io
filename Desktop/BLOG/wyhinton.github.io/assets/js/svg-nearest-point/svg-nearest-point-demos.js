...
function initializeLinearDemoWithPaths(svg, paths) {
  ...
  let linearSearchers = paths.slice(0, initialNumShapes).map((path, index) => new LinearPathSearch(path, currentSampleRate, index));
  ...
  if (container) {
    let rerandomizePathsAndUpdate2 = function() {
      ...
      linearSearchers = newTransformedPaths.slice(0, params.numShapes).map(
    ...
    var rerandomizePathsAndUpdate = rerandomizePathsAndUpdate2;
    const pane = new Pane({
      container,
      expanded: true,
    ...
    pane.addBinding(params, "sampleRate", {
      label: "Sample Rate",
      min: 50,
      max: 200,
      step: 10
      ...
      linearSearchers = paths.slice(0, params.numShapes).map(
        (path, index) => new LinearPathSearch(path, currentSampleRate, index)
      );
  ...
  svg.addEventListener("mousemove", function(e) {
    ...
    if (bestResult !== null && bestResult) {
      ...
      if (result.pathIndex !== void 0 && result.pathIndex < activeNumShapes && pathHighlights[result.pathIndex]) {
      ...
      const paneData2 = paneInstances.get("linear-performance-chart");
      if (paneData2) {
        paneData2.params.queryTime = combinedResult.time;
        paneData2.params.samplesChecked = combinedResult.samplesChecked || 0;
      }
    ...
    if (now - lastFpsUpdate > 1e3) {
      const fps = Math.round(frameCount * 1e3 / (now - lastFpsUpdate));
      const paneData2 = paneInstances.get("linear-performance-chart");
      if (paneData2) {
        paneData2.params.fps = fps;
      }
      frameCount = 0;
      lastFpsUpdate = now;
    }
...
var paneInstances2 = /* @__PURE__ */ new Map();
...
async function initializeQuadTreeDemo() {
...
function initializeQuadTreeDemoWithPaths(svg, paths) {
  ...
  function highlightContainingBoundary(containingBounds) {
    ...
    if (containingBounds) {
      allRects.forEach((rect) => {
        ...
        if (Math.abs(rectX - containingBounds.x) < 0.1 && Math.abs(rectY - containingBounds.y) < 0.1 && Math.abs(rectWidth - containingBounds.width) < 0.1 && Math.abs(rectHeight - containingBounds.height) < 0.1) {
  ...
  if (container) {
    let updateQuadTreeVisualization2 = function() {
    ...
    var updateQuadTreeVisualization = updateQuadTreeVisualization2;
    const pane = new Pane({
      container,
    ...
    paneInstances2.set("quadtree-performance-chart", { pane, params });
    updateQuadTreeVisualization2();
  }
  svg.addEventListener("mousemove", function(e) {
    const rect = svg.getBoundingClientRect();
    ...
    if (result.point) {
      ...
      connectionLine.setAttribute("y2", result.point.y.toString());
      connectionLine.style.display = "block";
      const paneData2 = paneInstances2.get("quadtree-performance-chart");
      if (paneData2 && paneData2.params.showQuadTree) {
        const containingBounds = quadTreeSearch.getContainingBounds(result.point);
    ...
    const paneData = paneInstances2.get("quadtree-performance-chart");
    if (paneData) {
