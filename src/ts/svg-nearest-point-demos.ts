...
interface SearchResult {
  ...
  pathIndex?: number; // Index of the path that contains this point
}
...
class LinearPathSearch {
  ...
  constructor(pathElement: SVGPathElement, sampleRate: number = 100, pathIndex: number = 0) {
...
async function initializeLinearDemo(): Promise<void> {
...
async function loadSVGPaths(svgUrl: string): Promise<string[]> {
  try {
...
function scaleSVGPath(pathData: string, scale: number, offsetX: number = 0, offsetY: number = 0): string {
...
function initializeLinearDemoFallback(svg: SVGElement): void {
...
function initializeLinearDemoWithPaths(svg: SVGElement, paths: SVGPathElement[]): void {
  ...
  const pathHighlights = paths.map((path, index) => {
    const highlightPath = document.createElementNS('http://www.w3.org/2000/svg', 'path') as SVGPathElement;
  ...
  // Update sample rate
  const sampleRateSlider = document.getElementById('sample-rate') as HTMLInputElement | null;
  const sampleCountSpan = document.getElementById('sample-count') as HTMLSpanElement | null;
...
function updateLinearPerformanceMetrics(result: SearchResult): void {
...
function initializeQuadTreeDemo(): void {
  ...
  // Controls
  const showQuadTreeCheckbox = document.getElementById('show-quadtree') as HTMLInputElement | null;
  const maxDepthSlider = document.getElementById('max-depth') as HTMLInputElement | null;
  const maxPointsSlider = document.getElementById('max-points') as HTMLInputElement | null;

  function updateQuadTree(): void {
...
function updatePerformanceChart(canvasId: string, data: number[], color: string): void {
...
function updateSampleVisualization(path: SVGPathElement, sampleRate: number, container: SVGGElement): void {
